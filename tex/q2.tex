...
\begin{document}
    ...
    \begin{enumerate}[label=\textbf{(\alph*)}]
        ...
        % \begin{itemize}
        %     \item Ordinal regression is a generalisation of logistic regression for ordinal data. It is used when the response variable is ordinal, i.e. it has a natural ordering. 
        %     \item Multiclass classification is used when the response variable is categorical, i.e. it has no natural ordering. 
        %     \item Linear regression is used when the response variable is continuous.
        % \end{itemize}
    ...
    \begin{align}
        \begin{split}
            ...
            % &= \sum_{j = 1}^{k} \sbrak{R_j \ln \brak{\frac{\boldsymbol{\kappa}_{j+1}}{\boldsymbol{\kappa}_j} \cdot \frac{1 + \boldsymbol{\kappa}_{j}}{1 + \boldsymbol{\kappa}_{j+1}}} 
            % - (R_{j + 1} - R_j) \ln \brak{\frac{\boldsymbol{\kappa}_j - \boldsymbol{\kappa}_{j + 1}}{\boldsymbol{\kappa}_{j}(1 + \boldsymbol{\kappa}_{j+1})}}} 
        \end{split}
    \end{align}
    ...
    We can use gradient descent to find the optimal weights \(\boldsymbol{\beta}\) or intervals \(\boldsymbol{\theta} = \myvec{\theta_1 & \ldots & \theta_{k - 1}}^T\). For \(\boldsymbol{\theta}\), 

    \begin{align}
        \begin{split}
            \nabla \ln p(\mathbf{y}|\mathbf{x}, \boldsymbol{\theta}) &= \sum_{j = 1}^{k} \sbrak{}
        \end{split}
    \end{align}
